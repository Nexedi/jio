...
(function (jIO, RSVP, DOMException, Blob, crypto, Uint8Array, ArrayBuffer) {
	...
	*/
	...
	function convertKey(that) {
		return new RSVP.Queue()
			.push(function () {
				...
				console.log("user_pass_phrase : " + passphraseKey);
			...
			.push(function (res) {
				console.log("Imported form AES-GCM : " + res);
				console.log(res);
				that._key = res;
				that._userkey = false;
				return;
		...
		return that._sub_storage.get(id)
			.push(function (blob) {
				...
				return new RSVP.Queue()
					...
					.push(function (coded) {
						console.log(coded.target.result);
						...
						return new RSVP.Queue()
							...
							.push(function (arr) {
								//arraybuffer->string
								console.log(arr);
								var string_from_string = String.fromCharCode.apply(null, new Uint8Array(arr));
								console.log(string_from_string);
								return JSON.parse(string_from_string);
	...
	jIO.addStorage('crypto_api2', CryptStorage_all);
