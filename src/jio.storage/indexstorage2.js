...
(function (indexedDB, jIO, RSVP, IDBOpenDBRequest, DOMError,
  ...
  function getDocs(storage) {
    ...
    try {
      ...
      if ((error1 instanceof jIO.util.jIOError) &&
          (error1.status_code === 501)) {
        try {
          ...
          return storage.allDocs()
            ...
            .push(function (temp_result) {
              var bing_result = {data: {rows: []}}, keys;
              keys = Object.keys(temp_result);
              for (i = 0; i < keys.length; i += 1) {
                bing_result.data.rows.push({id: keys[i],
                  doc: temp_result[keys[i]]});
              }
              bing_result.data.total_rows = bing_result.data.rows.length;
              return bing_result;
            });
        } catch (error2) {
          if ((error2 instanceof jIO.util.jIOError) &&
              (error2.status_code === 501)) {
            throw new jIO.util.jIOError("Can't repair because substorage" +
              " doesn't support buildQuery");
          }
          throw error2;
        }
      }
