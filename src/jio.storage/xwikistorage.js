...
var newXWikiStorage = function(spec, my)
{
    ...
    var escapeDocId = function(docId) {
        return docId.replace(/.html$/, '').split('/').join('%2F');
    };

    /** Restore a document id from the escaped form. */
    var restoreDocId = function(escapedDocId) {
     ...
     *
     * @param docId the document id of a document which you have permission to edit.
     * @param whatToDo a function which will be called with the form token as the parameter.
     */
    var doWithFormToken = function(docId, whatToDo) {
        var url = priv.xwikiurl + '/bin/edit/' + priv.space + '/' + escapeDocId(docId) +
                '?editor=wiki&cachebuster=' + Date.now();
        $.ajax({
            url: url,
            type: "GET",
            async: true,
     ...
     * @param callWhenDone a callback which will be called when this function finishes.
     */
    var getDates = function(docId, callWhenDone) {
        // http://127.0.0.1:8080/xwiki/rest/wikis/xwiki/spaces/Main/pages/<pageName>
        var map = {};
        $.ajax ( {
            url: priv.xwikiurl + '/rest/wikis/' + 'xwiki' +
                '/spaces/' + priv.space +
        ...
        doc = (function() {
            ...
            $.ajax({
                url: priv.xwikiurl + '/bin/get/' + priv.space +
                    '/' + escapeDocId(command.getDocId()) +
                    '?xpage=plain&cachebuster=' + Date.now(),
                type: "GET",
                async: true,
                dataType: 'text',
                headers: {'Authorization':'Basic ' + Base64.encode(
