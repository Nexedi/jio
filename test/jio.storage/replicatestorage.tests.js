/*jslint nomen: true*/
/*global Blob, console, RSVP, setTimeout*/
(function (jIO, QUnit, Blob, console, RSVP, setTimeout) {
  "use strict";
  var test = QUnit.test,
    stop = QUnit.stop,
    start = QUnit.start,
...
//        equal(error.message, "Cannot find document: conflict");
  ...
  test("parallel operation 2", function () {
    stop();
    expect(1);
    ...
    Storage2713.prototype.getAttachment = function (id) {
      ...
      return new RSVP.Promise(function (resolve, reject) {
        if (id === "3") {
          if (!(start_sync[1] && start_sync[2])) {
            console.log(start_sync);
            reject(new jIO.util.jIOError("Not paralelle", 404));
          }
        }
    ...
    return context.jio.put("0", {"title": "foo"})
      ...
      .then(function () {
        equal(sync_pause, "false,false,true,true", "rigth order");
      })
      .fail(function (error) {
        console.log(error);
        ok(error instanceof jIO.util.jIOError);
...
}(jIO, QUnit, Blob, console, RSVP, setTimeout));