...
(function (jIO, QUnit, indexedDB, Blob) {
  ...
  test("Parallel operation", function () {
    var context = this, other_storage, fake_data;
    stop();
    expect(3);
    ...
    function checkConflict() {
      ...
      context.jio = new_storage;
      return new_storage.allDocs({query: "a:55"})
        .push(function (result) {
          equal(result.data.rows.length, 1);
        });
    }

    DummyStorage3.prototype.put = function (id, value) {
      fake_data[id] = {id: id, doc: value};
      return id;
    };
    ...
    DummyStorage3.prototype.buildQuery = function () {
      return new RSVP.Promise(function (resolve) {
        setTimeout(function () {
          resolve(Object.values(fake_data));
        }, 1000);
      });
    };

    other_storage = jIO.createJIO({
      type: "dummystorage3"
    });
    other_storage.put("42", {"a": "55", "b": "2"})
      .then(function () {
        return RSVP.all([checkConflict(), checkConflict(), checkConflict()]);
      })
      .fail(function (error) {
        console.log(error);
      })
