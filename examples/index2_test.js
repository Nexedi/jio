...
/*  function randomi(limit) {
  ...
  function get_fake_data_values2(i) {
    if (i === 0 || i === 1 || i === 2) {
      return {'url': 'https://google.com', 'name': 'google', 'user': 'preet'};
    }
    if (i === 3 || i === 4) {
      return {'url': 'https://google.com', 'name': 'google', 'user': 'test'};
    }
    if (i === 5 || i === 6 || i === 7) {
      return {'url': 'https://nexedi.com', 'name': 'nexedi', 'user': 'preet'};
    }
    return {'url': 'https://jio.nexedi.com', 'name': 'jio', 'user': 'obscure'};
  }
  ...
  rJS(window)
    ...
    .declareService(function () {
      var storage = jIO.createJIO({
        ...
        index_key: "short_description",
        sub_storage: {
          type: "indexeddb",
          database: "index2testdata",
        }
      ...
      return RSVP.all(promise_list)
        .then(function () {
          ...
          return storage.allDocs({query: 'name:google AND user:preet'})
            .then(function (result) {
              console.log('Time to query 1 - ');
              console.log(performance.now() - time2);
              console.log(result);
              console.log('Query 2');
              var time3 = performance.now();
              return storage.allDocs({query: 'name:jio OR name:google'})
                .then(function (result) {
                  ...
                  return storage.allDocs({query: 'user:string OR name:nexedi'})
                    .then(function (result) {
                      console.log('Time to query 3 - ');
                      console.log(performance.now() - time4);
                      console.log(result);
                      console.log('Query 4');
                      var time5 = performance.now();
                      return storage.allDocs({query: 'user:string OR name:not'})
                        .then(function (result) {
                          console.log('Time to query 4 - ');
                          console.log(performance.now() - time5);
                          console.log(result);
                        });
                    });
                });
